class LastUpdatesController < ApplicationController
  ...
  def api_additional_fields_to_merge(last_update)
    adress_book = AdressBook.find_by_id(last_update["user_id"])
    return {name: "#{(adress_book.full_name rescue '')}", company: "#{(Company.find_by_id(last_update['company_id']).name rescue '')}", post: "#{(adress_book.job_role rescue '')}"}
  end
